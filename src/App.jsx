...
export default function App(){
  ...
  if(!user){
    ...
    if(authMode==='role')return(<div style={S}><div style={{width:'100%'}}><div style={{textAlign:'center',marginBottom:32}}><div style={{fontSize:52}}>🌿</div><div style={{fontSize:26,fontWeight:800,color:BK}}>HEHA<span style={{color:OR}}>·</span>swipe</div><div style={{color:'#888',fontSize:16,marginTop:4}}>Tampa's healthy business community</div></div><div style={{marginBottom:12}}><button onClick={()=>setAuthMode('signup')} style={{...btn(BK),marginBottom:8}}>I'm a Local User</button><button onClick={()=>setAuthMode('partner')} style={{...btn('transparent','#555'),border:'1.5px solid #ccc',marginBottom:0}}>I'm a Local Business</button></div><div style={{textAlign:'center',marginTop:8}}><button onClick={()=>setAuthMode('signin')} style={{background:'none',border:'none',color:'#888',fontSize:14,cursor:'pointer'}}>Already have an account? Sign in</button></div></div></div>)

...
function getPartnerTags(partner) {
  const tags = [
    ...
    .filter(Boolean)
    .filter((tag) => !String(tag).includes("crm-seed"));
...
export default function SwipeCard({ partner, onSwipe }) {
  ...
  const moveDrag = (clientX, clientY) => {
    ...
    if (Math.abs(nextDrag.x) > 8 || Math.abs(nextDrag.y) > 8) setTouchMoved(true);
    setDrag(nextDrag);
  };
  ...
  const openPreview = () => {
    if (touchMoved || Math.abs(drag.x) > 8 || Math.abs(drag.y) > 8) return;
    setShowPreview(true);
  };
      ...
      >
        ...
        <div className="card-bottom-zone">
          ...
          <div className="card-footer compact-footer clean-footer">
            ...
            <button type="button" onClick={(event) => { event.stopPropagation(); setShowPreview(true); }}>Preview</button>
          </div>
        </div>
...
function PartnerPreviewSheet({ partner, categoryGroup, images, tags, items, onClose, onSave, onPass, onShare }) {
  ...
  return (
    <div className="preview-backdrop" role="dialog" aria-modal="true" aria-label={`${partner.name} preview`}>
      <section className="partner-preview-sheet">

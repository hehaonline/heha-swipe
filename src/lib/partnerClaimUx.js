...
export const CLAIM_SUCCESS_VERSION = 2;
export const CLAIM_SUCCESS_MAX_AGE_MS = 15 * 60 * 1000;

export function claimSessionStorage(host) {
...
export const CLAIM_ERROR_DETAILS = Object.freeze({
  ...
  HEHA_CLAIM_SESSION_CHANGED: "Your account changed while checking this claim. Sign in with the invited account and check the link again.",
  HEHA_CLAIM_INVALID_RESPONSE: "HEHA could not confirm the claim response. Check your account before trying this one-time link again, or contact support.",
  HEHA_CLAIM_SUBMISSION_PENDING: "This claim is already being processed. Check your account before trying again.",
});
...
export function saveClaimSuccess(storage, partnerName, profileSetupPending, userId, now = Date.now(), partnerId) {
  const cleanPartnerName = typeof partnerName === "string" ? partnerName.trim() : "";
  if (!cleanPartnerName || cleanPartnerName.length > 200 || !UUID_PATTERN.test(userId || "") || !UUID_PATTERN.test(partnerId || "")) return false;
...
export function readClaimSuccess(search, storage, expectedUserId, now = Date.now()) {
  ...
  try {
    ...
    if (!UUID_PATTERN.test(stored.partnerId || "")) return null;
...
export function consumeClaimSuccess(search, storage, expectedUserId, now = Date.now()) {
  ...
  if (isSuccessReturn) clearClaimSuccess(storage);
  return success;
}

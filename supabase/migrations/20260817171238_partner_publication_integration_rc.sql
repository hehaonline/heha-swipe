...
begin
  ...
  if unexpected_policy is not null then
    raise exception using
      errcode='55000',
      message=pg_catalog.format(
        'Unexpected public.partners DML/FOR ALL policy requires review: %s',
        unexpected_policy
      );
  end if;
...
declare
  ...
  account_is_eligible boolean := false;
  current_partner public.partners%rowtype;
  updated_partner public.partners%rowtype;
  previous_hash text;
  current_hash text;
  normalized_categories text[];
  normalized_tags text[];
  normalized_offerings text[];
  normalized_delivery_days text[];
begin
  if actor_id is null then
    raise exception using
      errcode='42501',
      message='Sign in before editing a partner profile.';
  end if;

  select
    not coalesce(user_row.is_anonymous,false)
    and (
      user_row.email_confirmed_at is not null
      or user_row.phone_confirmed_at is not null
    )
  into account_is_eligible
  from auth.users user_row
  where user_row.id=actor_id;

  if not coalesce(account_is_eligible,false) then
    raise exception using
      errcode='42501',
      message='A verified non-anonymous account is required.';
  end if;
  ...
  if p_expected_profile_snapshot_hash is null
    ...
    raise exception using
      errcode='22023',
      message='The exact current partner profile hash is required.';
  end if;

  if nullif(pg_catalog.btrim(p_name),'') is null
     or pg_catalog.octet_length(p_name)>320 then
    raise exception using
      errcode='23514',
      message='Partner name is required and must not exceed 320 bytes.';
  end if;

  normalized_categories := app_private.normalize_partner_categories(
    p_categories,
    null
  );
  if pg_catalog.cardinality(normalized_categories)=0
     or pg_catalog.cardinality(normalized_categories)>8
     or exists (
       select 1
       from pg_catalog.unnest(normalized_categories) category_value
       where category_value<>all(array[
    ...
    raise exception using
      errcode='23514',
      message='Choose one to eight supported business categories.';
  end if;

  select coalesce(
    pg_catalog.array_agg(
      pg_catalog.btrim(array_value.value)
      order by array_value.ordinality
    ) filter (
      where nullif(pg_catalog.btrim(array_value.value),'') is not null
    ),
    array[]::text[]
  )
  ...
  ) with ordinality as array_value(value,ordinality);
  ...
  into normalized_delivery_days
  from pg_catalog.unnest(
    coalesce(p_delivery_days,array[]::text[])
  ) with ordinality as array_value(value,ordinality);

  if pg_catalog.cardinality(normalized_tags)>30
     or pg_catalog.cardinality(normalized_offerings)>30
     or pg_catalog.cardinality(normalized_delivery_days)>7 then
    raise exception using
      errcode='23514',
  ...
  if pg_catalog.octet_length(coalesce(p_neighborhood,''))>1000
     ...
     or pg_catalog.octet_length(coalesce(p_hours,''))>2000
     or pg_catalog.octet_length(coalesce(p_business_type,''))>1000
     or pg_catalog.octet_length(coalesce(p_website,''))>2000
     or pg_catalog.octet_length(coalesce(p_instagram,''))>1000
     or pg_catalog.octet_length(coalesce(p_price_range,''))>1000 then
    raise exception using
      errcode='23514',
      message='One or more partner profile text fields exceed the bounded limit.';
  end if;
  ...
  previous_hash := app_private.partner_public_profile_hash(current_partner);
  if previous_hash is distinct from p_expected_profile_snapshot_hash then
    raise exception using
      errcode='40001',
  ...
  update public.partners partner_row
  set
    name=pg_catalog.btrim(p_name),
    category=normalized_categories[1],
    categories=normalized_categories,
    neighborhood=nullif(pg_catalog.btrim(coalesce(p_neighborhood,'')),''),
    tagline=nullif(pg_catalog.btrim(coalesce(p_tagline,'')),''),
    bio=nullif(pg_catalog.btrim(coalesce(p_bio,'')),''),
    tags=normalized_tags,
    hours=nullif(pg_catalog.btrim(coalesce(p_hours,'')),''),
    business_type=nullif(pg_catalog.btrim(coalesce(p_business_type,'')),''),
    offerings=normalized_offerings,
    website=nullif(pg_catalog.btrim(coalesce(p_website,'')),''),
  ...
  where partner_row.id=p_partner_id
    and partner_row.owner_id=actor_id
  returning partner_row.* into updated_partner;

  perform pg_catalog.set_config('app.hybrid_partner_context','',true);

  current_hash := app_private.partner_public_profile_hash(updated_partner);
  if current_hash is not distinct from previous_hash then
    raise exception using
      errcode='22023',
      message='Partner profile replacement did not change the public snapshot.';
  end if;

  return pg_catalog.jsonb_build_object(
    'partner_id',updated_partner.id,
    'previous_profile_snapshot_hash',previous_hash,
    'profile_snapshot_hash',current_hash,
    'profile_snapshot',app_private.partner_public_profile_snapshot(
      updated_partner
    ),
...
revoke all on function public.update_my_partner_profile(
  uuid,text,text,text[],text,text,text,text[],text,text,text[],text,text,text,text[]
) from public,anon,authenticated,service_role;
grant execute on function public.update_my_partner_profile(
  uuid,text,text,text[],text,text,text,text[],text,text,text[],text,text,text,text[]
) to authenticated;
...
begin
  ...
  if actual_policies is distinct from expected_policies then
    raise exception using
      errcode='55000',
      message=pg_catalog.format('Unexpected public.partners policy set: %s',actual_policies);
  end if;
  ...
  if not pg_catalog.has_table_privilege('authenticated','public.partners','SELECT')
    ...
    raise exception using
      errcode='55000',
      message='authenticated public.partners ACL is not exactly SELECT-only.';
  end if;
  ...
  if pg_catalog.has_any_column_privilege('anon','public.partners','SELECT')
     ...
     or pg_catalog.has_any_column_privilege('authenticated','public.partners','INSERT')
     or pg_catalog.has_any_column_privilege('authenticated','public.partners','UPDATE')
  ...
  if pg_catalog.to_regprocedure(
     ...
     or exists (
       ...
       where function_row.oid=pg_catalog.to_regprocedure(
         ...
         and (
           privilege_row.grantee=0
           or role_row.rolname is distinct from 'authenticated'
           or privilege_row.is_grantable
         )
     ) then
    raise exception using
      errcode='55000',
      message='Owner profile RPC is not an authenticated-only, typed SECURITY DEFINER boundary.';
  end if;
  ...
  if exists (
    ...
    where protected_function.oid=any(array[
      ...
      'public.update_my_partner_profile(uuid,text,text,text[],text,text,text,text[],text,text,text[],text,text,text,text[])'::regprocedure
    ]::oid[])

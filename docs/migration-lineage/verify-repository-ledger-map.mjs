...
if (inventory.length !== 49) fail(`repository inventory: expected 49 rows, got ${inventory.length}`);
if (liveMap.length !== 96) fail(`live compatibility map: expected 96 rows, got ${liveMap.length}`);
if (repoMap.length !== 49) fail(`repository disposition map: expected 49 rows, got ${repoMap.length}`);
if (repositoryExpectations.length !== 49) {
  fail(`reviewed repository expectations: expected 49 rows, got ${repositoryExpectations.length}`);
}
...
const expectedRepoCounts = { AN: 1, AR: 1, AS: 1, BC: 40, BS: 6 };

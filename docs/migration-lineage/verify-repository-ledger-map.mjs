...
if (inventory.length !== 48) fail(`repository inventory: expected 48 rows, got ${inventory.length}`);
if (liveMap.length !== 96) fail(`live compatibility map: expected 96 rows, got ${liveMap.length}`);
if (repoMap.length !== 48) fail(`repository disposition map: expected 48 rows, got ${repoMap.length}`);
if (repositoryExpectations.length !== 48) {
  fail(`reviewed repository expectations: expected 48 rows, got ${repositoryExpectations.length}`);
}
...
const expectedRepoCounts = { AN: 1, AR: 1, AS: 1, BC: 39, BS: 6 };

...
- the current **49-file / 45-version** `supabase/migrations` tree;
...
## Repository-file class codes — 49 files

| Code | Files | Canonical disposition |
|---|---:|---|
| `BC` | 40 | Candidate component only after deep metadata, normalized-effect, dependency, and security review. |
...
There are **zero exact version matches** between the 96 live versions and the 45 unique versions in the current executable migration tree.
...
- `repository-migration-disposition-map-2026-08-24.csv` — all 49 current executable files.
...
## September 16 shared media boundary extension

PR #141 adds one further RP/BC forward candidate, with a blank live-ledger edge, bringing the current packet to 49 files / 45 versions. It closes review findings on active path/slot duplication and concurrent gallery intake. Historical September 14 counts and all historical migration bytes remain intact. Existing active conflicts or boundary drift must fail closed for independently reviewed resolution; this packet does not authorize hosted data cleanup or application.

# Partner publication-permission review with Wave 1 HEHA Local
...
Scope: explicit HEHA Swipe profile permission for the supported Restaurant, Vendor, Catering, PrivateChef, Wellness, Coach, Service, and Events categories. HEHA Local remains a separately activated Wave 1 destination limited to Tampa Bay Catering and PrivateChef partners; arbitrary legacy categories fail closed.

## What this slice establishes

- A signed-in partner can submit one profile and explicitly choose HEHA Swipe; Catering and PrivateChef profiles may also choose HEHA Local. Nothing is preselected.
- Every existing owned partner profile in a supported category can enter the Swipe permission flow without creating a duplicate listing; only Catering and PrivateChef profiles are offered Local, and arbitrary legacy categories fail closed.
- Private profile preparation and exact-version publication approval are separate actions.
- The server records structurally append-only private consent evidence with representative, destination, media permission, statement version, server snapshot/hash, timestamp, actor, source, and idempotency data. Tampa Bay service-area attestation and a Local lane are recorded only for Local. Owner/recorder UUIDs are durable evidence snapshots rather than `ON DELETE SET NULL` Auth foreign keys, so account deletion cannot rewrite history; retention or anonymization requires a separately approved release policy. Browser roles—including current and former owners—cannot select the evidence ledger directly; owners receive only redacted current status through an ownership-checked RPC.
...
- Every Swipe or Local profile is consent-managed from its first draft. Changing categories cannot reuse stale evidence or fall back into a legacy public route.
...
- An exact-current-hash HEHA staff review may move a pending listing only to approved; it never moves the listing to live. Profile preparation or owner publication consent never performs the lifecycle transition by itself.
- The HEHA Swipe public projection remains intentionally empty until founder/legal approves immutable partner-terms and privacy versions and a successor binds both explicit acknowledgements to the private evidence contract.
...
- A submitted `partners` row remains the durable review-queue record. No browser webhook participates in this authorization or review boundary.

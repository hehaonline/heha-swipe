...
- Phase A is additive and applies only to identified callers of the 13-field
  `list_public_swipe_partner_cards`, 24-field
  `list_public_swipe_partner_details`, or 17-field
...
- Phase B removes `public`, `anon`, and `authenticated` access to all three
  ...
  `authenticated` only SELECT/INSERT/UPDATE table privileges on `partners`,
  constrained by RLS. Owner self-service uses direct row operations; the
  internal routing UI reads directly but writes only through its role-checked
  SECURITY DEFINER RPCs. The relation owner and `service_role` remain untouched.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| HEHA Swipe store-card reader | Healthy Habit LLC / GitHub `hehaonline/heha-swipe`; Supabase project ID unrecorded | Native store candidate; hosted target unverified | Supabase public anon key; optional authenticated user JWT | Unassigned | Code cut over; live RPC unavailable/unverified | `src/App.jsx` passes the native/store release policy to `src/lib/publicPartner.js`, which calls `list_public_swipe_partner_cards`; contract tests cover 13 fields | Unknown; no dated hosted smoke receipt | NOT CERTIFIED — prove exact fields and eligible IDs as anon/auth in approved staging | NOT CERTIFIED — prove card loading after legacy closure without fallback | NOT CERTIFIED — no pricing disposition approved | No `heha_pricing` call found in this source path; no dated non-use proof |
| HEHA Swipe web-detail reader | Healthy Habit LLC / GitHub `hehaonline/heha-swipe`; Supabase project ID unrecorded | Ordinary HEHA Swipe web client; hosted revision unverified | Supabase public anon key; optional authenticated user JWT | Unassigned | Code cut over; live RPC unavailable/unverified | `src/App.jsx` explicitly selects `list_public_swipe_partner_details` through `src/lib/publicPartner.js`; contract tests cover 24 fields, nested item/gallery whitelists, HTTP(S)-only item links, and generic-lane denial | Unknown; no dated hosted smoke receipt | NOT CERTIFIED — prove exact fields, nested whitelists, and eligible IDs as anon/auth in approved staging | NOT CERTIFIED — prove gallery, item, HEHA Local, CTA, and social behavior after legacy closure | NOT CERTIFIED — no pricing disposition approved | No `heha_pricing` call found in this source path; no dated non-use proof |
...
| Make scenarios | Healthy Habit LLC intended / Make organization identifier unverified | Scenario names and environments unknown | Connection and database role unknown | Unassigned | External scenarios/connections uninspected | No scenario export, connection inventory, run receipt, or owner attestation | Unknown | N/A — not an identified caller of a Phase-A RPC | NOT CERTIFIED — exact caller and bounded replacement or non-use required | NOT CERTIFIED — direct pricing dependency unknown | No scenario export, dated run receipt, or owner-attested non-use |

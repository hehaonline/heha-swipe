...
- the hostname returned by a strict HTTP(S) URL parse, with `www.` and a trailing dot removed, for domains; query/path text never counts as the host, queries/fragments are rejected, and neither fixture hostnames nor paths can contain digits that could conceal phone/account-shaped payloads;

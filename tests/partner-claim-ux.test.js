...
test("blocked browser storage cannot turn a committed claim into a false failure", () => {
  ...
  assert.equal(saveClaimSuccess(blockedStorage, "Local Business", false, USER_A, 1_000_000), false);
  assert.equal(consumeClaimSuccess("?claim=success", blockedStorage, USER_A, 1_000_000), null);
  assert.equal(clearClaimSuccess(blockedStorage), false);
});
...
test("claim UI includes persistent, review-gated, non-public confirmation copy", async () => {
  ...
  assert.doesNotMatch(screen, /subscription_type/);
  assert.doesNotMatch(screen, /from\("profiles"\)/);
  assert.doesNotMatch(screen, /localStorage\.setItem/);
  assert.match(screen, /htmlFor="claim-business-email"/);
